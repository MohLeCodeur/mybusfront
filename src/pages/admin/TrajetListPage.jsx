...
const TrajetListPage = () => {
  ...
  const ITEMS_PER_PAGE = 6; // 6 cartes par page pour un design aéré
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentTrajets.length > 0 ? currentTrajets.map(t => {
                ...
                // --- LOGIQUE DE STATUT CALCULÉE ICI ---
                const departureDateTime = new Date(`${new Date(t.dateDepart).toISOString().split('T')[0]}T${t.heureDepart}:00Z`);
                const tripEndTime = new Date(departureDateTime.getTime() + (12 * 60 * 60 * 1000));
                const isFinished = new Date() > tripEndTime;
                // ------------------------------------

                return (
                  <div key={t._id} className={`p-4 rounded-lg border-l-4 ${t.isActive ? 'border-blue-500' : 'border-gray-300'} bg-white shadow-md hover:shadow-xl hover:-translate-y-1 transition-all`}>
                      <div className="flex justify-between items-start">
                          <div className="font-bold text-gray-800 text-lg flex items-center gap-2"><FaRoute/> {t.villeDepart} → {t.villeArrivee}</div>
