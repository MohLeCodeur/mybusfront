...
const ReservationListPage = () => {
  ...
  const handleDelete = async (id) => {
    if (window.confirm(`Supprimer la réservation ID: ${id} ?\nCette action est irréversible et libérera les places.`)) {
      try {
                ...
                {reservations.length > 0 ? (
                  reservations.map(r => (
                    <tr key={r._id}>
                      ...
                      <td className="px-6 py-4">{new Date(r.dateReservation).toLocaleDateString('fr-FR')}</td>
                      <td className="px-6 py-4 text-center">{r.placesReservees}</td>
                      <td className="px-6 py-4"><span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusBadge(r.statut)}`}>{r.statut.replace('_', ' ')}</span></td>
