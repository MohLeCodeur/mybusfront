...
const ConfirmationPage = () => {
  ...
  const handleDownloadTicket = async () => {
    ...
    // On sauvegarde la largeur originale pour la restaurer plus tard
    const originalWidth = ticketNode.style.width;
    // On force la largeur pour la capture pour éviter les problèmes de découpe
    ticketNode.style.width = `${ticketNode.offsetWidth}px`;

    try {
      const dataUrl = await toPng(ticketNode, {
        quality: 1.0,
        pixelRatio: 2.5, // Haute résolution pour une qualité d'impression parfaite
        backgroundColor: '#f9fafb', // Couleur de fond de la page
      });
      ...
      const imgWidth = pdfWidth - 20; // 10mm de marge de chaque côté
      ...
      // On restaure la largeur originale de l'élément
      ticketNode.style.width = originalWidth;
    ...
    <main className="py-16 px-4 min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <div className="mt-6 bg-blue-50 border-l-4 border-blue-500 text-blue-800 p-4 rounded-r-lg max-w-lg mx-auto text-left"><div className="flex items-center gap-3"><FiMapPin className="text-2xl"/><dvi>
              <h4 className="font-bold">Suivi en temps réel disponible</h4>
              <p className="text-sm">Vous pourrez suivre votre bus depuis votre <Link to="/dashboard" className="font-semibold underline hover:text-blue-600">espace "Mon Compte"</Link> dès que le voyage aura commencé.</p>
          </dvi></div></div>
        </div>
