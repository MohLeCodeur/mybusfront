...
const NextTripWidget = ({ tripData }) => {
  ...
  const now = new Date();
  const isFuture = departureDateTime > now;
  
  // Calculer si le voyage est dans la fenêtre de suivi (5 heures après le départ)
  const fiveHoursAfterDeparture = new Date(departureDateTime.getTime() + (5 * 60 * 60 * 1000)); // +5 heures
  const isInTrackingWindow = now >= departureDateTime && now <= fiveHoursAfterDeparture;
  // ----------------------------------------
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              {isFuture ? "Prochain départ" : 
               isInTrackingWindow ? "Voyage en cours" : 
               "Voyage récent"}
            </p>
            {/* ----------------------- */}
        ...
        {isFuture ? (
          ...
          <div className="text-center mt-4">
            {/* --- CONDITION MODIFIÉE POUR AFFICHER LE BOUTON PENDANT 5H --- */}
            {tripData.liveTrip && isInTrackingWindow ? (
              <div className="space-y-3">
                <Link 
                  to={`/tracking/map/${tripData.liveTrip._id}`} 
                  className="inline-block px-8 py-3 bg-green-500 text-white font-bold rounded-lg hover:bg-green-600 transition shadow-lg hover:shadow-green-400/50"
                >
                ...
                <div className="text-xs text-gray-500">
                  Suivi disponible jusqu'à {fiveHoursAfterDeparture.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
            ) : tripData.liveTrip && !isInTrackingWindow ? (
              // Voyage terminé (plus de 5h après le départ)
              <div className="p-4 bg-gray-100 rounded-lg flex items-center justify-center gap-3 text-gray-600">
                <FiCheckCircle className="text-green-500"/>
                <p>Voyage terminé</p>
              </div>
            ) : (
              // Pas de LiveTrip disponible
              <div className="p-4 bg-gray-100 rounded-lg flex items-center justify-center gap-3 text-gray-600">
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={handleTrack} className="flex gap-2">
        <input 
          ...
          className="flex-grow border p-2 rounded-md focus:ring-2 focus:ring-pink-400"
        />
        <button 
          type="submit" 
          disabled={loading} 
          className="p-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-300"
        >
...
const ClientDashboardPage = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-pink-50 via-blue-50 to-white min-h-screen">
      <div className="max-w-7xl mx-auto py-16 px-4">
        <div className="mb-12">
          ...
          <p className="text-lg text-gray-500 mt-2">Gérez vos voyages et colis en un seul endroit.</p>
        </div>
