...
const CountdownDisplay = ({ targetDateTime }) => {
    ...
    const d=Math.floor(timeLeft/(1000*60*60*24)), h=Math.floor((timeLeft/(1000*60*60))%24), m=Math.floor((timeLeft/1000/60)%60), s=Math.floor((timeLeft/1000)%60);
    ...
    if (!data || !data.reservation) {
        return (
            <div className="text-center p-8 bg-gray-50/50 rounded-lg">
                ...
                <Link to="/search" className="inline-block px-6 py-2 bg-gradient-to-r from-pink-500 to-blue-500 text-white font-semibold rounded-lg hover:shadow-lg transition">Réserver un voyage</Link>
            </div>
        );
    }
        ...
        <div className="relative bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
            <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                    ...
                    {data.liveTrip && (
                        <Link to={`/tracking/map/${data.liveTrip._id}`} className="px-4 py-2 bg-green-500 text-white font-bold rounded-lg hover:bg-green-600 text-sm"><FiMapPin className="inline mr-1"/> Suivre</Link>
                    )}
                </div>
                {isFuture ? (
                    ...
                    <div className="text-center mt-4">
                        {data.liveTrip ? (
                            <Link to={`/tracking/map/${data.liveTrip._id}`} className="inline-block px-8 py-3 bg-green-500 text-white font-bold rounded-lg hover:bg-green-600 transition shadow-lg hover:shadow-green-400/50 text-base"><FiMapPin className="inline mr-2"/> Suivre en Temps Réel</Link>
                        ) : (
                            <div className="p-4 bg-gray-100 rounded-lg flex items-center justify-center gap-3 text-gray-600"><p>{data.message || "Le suivi pour ce voyage est indisponible."}</p></div>
                        )}
                    </div>
                )}
            </div>
...
const ClientDashboardPage = () => {
    ...
    useEffect(() => {
        const fetchData = () => { api.get('/dashboard/client-data').then(res => setDashboardData(res.data)).catch(console.error).finally(() => { if (loading) setLoading(false); }); };
        fetchData();
