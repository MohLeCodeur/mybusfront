...
const TrackingMapPage = () => {
    ...
    useEffect(() => {
        const fetchTripData = () => {
            api.get(`/tracking/live/${liveTripId}`)
                .then(res => {
                    console.log("Données LiveTrip reçues:", res.data); // LOG DE DÉBOGAGE
        ...
        fetchTripData();
        const refreshInterval = setInterval(fetchTripData, 15000);
    ...
    const startSimulation = () => {
        ...
        simulationIntervalRef.current = setInterval(() => {
            const coords = liveTrip.routeGeoJSON.coordinates;
            ...
            setLiveTrip(prev => ({ ...prev, currentPosition: { lat, lng }, lastUpdated: new Date() }));
            currentIndex++;
        }, 1000);
    };
    ...
    if (!liveTrip) return <div className="text-center p-8">Aucune information de suivi trouvée.</div>;
    ...
    return (
        <div className="h-screen w-screen grid grid-cols-1 lg:grid-cols-4">
            <aside className="lg:col-span-1 bg-white p-4 flex flex-col shadow-lg z-[1000] overflow-y-auto">
                ...
                <div className="shrink-0">
                    ...
                    <p className={`text-sm font-bold mt-1 ${liveTrip.status === 'En cours' ? 'text-green-600 animate-pulse' : 'text-gray-600'}`}>{liveTrip.status}</p>
                </div>

                <div className="my-4 border-t pt-4 shrink-0">
                    {liveTrip.routeSummary?.distanceKm ? (
                        <div className="space-y-2 text-sm">
                            <p className="flex items-center gap-2"><FiMap /> <strong>Distance:</strong> {liveTrip.routeSummary.distanceKm} km</p>
                            <p className="flex items-center gap-2"><FiClock /> <strong>Durée:</strong> {Math.floor(liveTrip.routeSummary.durationMin / 60)}h {liveTrip.routeSummary.durationMin % 60}min</p>
                        </div>
                    ) : ( <p className="text-sm text-gray-400">Aucun itinéraire calculé.</p> )}
                </div>
                ...
                <div className="my-4 border-t pt-4 flex-grow min-h-0 flex flex-col">
                    <h2 className="font-bold mb-2 shrink-0">Instructions</h2>
            ...
            <main className="lg:col-span-3 z-0">
                <MapContainer center={busPosition || startPosition || [12.63, -8.00]} zoom={8} className="h-full w-full">
                    {busPosition && <ChangeView center={busPosition} />}
